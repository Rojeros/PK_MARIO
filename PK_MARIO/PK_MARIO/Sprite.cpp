...
void Sprite::DrawCurrentFrame(double x, double y, double width, double height) {
	...
	//
	// poni¿ej znajduje siê kod dla rysowania z w³¹czonym powtarzaniem
	//

	const double rep_width = m_width_repeat;                           // rozmiary powtarzanego sprite'a
	const double rep_height = m_height_repeat;
	const int whole_in_x = static_cast<int>(width / rep_width);        // liczba ca³ych sprite'ów na ka¿dej z osi
	const int whole_in_y = static_cast<int>(height / rep_height);
	const double scrap_width = width - whole_in_x * rep_width;         // rozmiar skrawków (czêœci brakuj¹cych do pe³nych klocków)
	...
	// dla ka¿dego y'ka narysuj skrawki x'ów
	...
	// dla ka¿dego x'a narysuj skrawki y'ów
