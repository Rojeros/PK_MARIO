...
public:
	Aabb(double min_x, double min_y, double max_x, double max_y) :
		...
		// mo¿na zamieniæ wartoœci zamiast sprawdzaæ asercjê, ale
		// b³êdne przekazanie argumentów mog³o byæ skutkiem
		// b³êdu podczas tworzenia obiektu (Ÿle podane argumenty)

		assert(m_min_x < m_max_x);
		assert(m_min_y < m_max_y);

		// EDIT: asercje faktycznie przyda³y siê parokrotnie ;-)
	}
