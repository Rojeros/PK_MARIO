...
void Renderer::LoadTexture(const std::string & filename) {
	...
	if (!surface) {
		std::cerr << "£adowanie pliku " + filename + " FAILED: " + SDL_GetError() + "\n";
		exit(1);
	}
	...
	if (((width & (width - 1)) != 0) || ((height & (height - 1)) != 0)) {
		std::cerr << "Obrazek " + filename + " ma nieprawid³owe wymiary (powinny byæ potêg¹ 2): " << width << "x"
			<< height << "\n";
		exit(1);
	}
	...
	glTexImage2D(GL_TEXTURE_2D, 0, surface->format->BytesPerPixel, width, height, 0, format, GL_UNSIGNED_BYTE,
		surface->pixels);
	if (surface) {
